...
thereby try to tailor the model output.
In regard to LLMs, we selected different categories based on related work in the case of the
prompting setting, as well as categories to classify interactions into common use cases.
Following this approach will provide insights if for example longer, more complex queries with a
lot of modifiers improve the results or have no beneficial effect.
While existing taxonomies often classify one individual prompt into its various parts, such as ``
subject term'', ``style modifier'', or ``quality booster'' \cite{oppenlaender_taxonomy_2023}, we
