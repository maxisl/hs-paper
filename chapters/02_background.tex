...
Interest in query optimization in order to improve output is widely prevalent ever since the broad
...
Even though their focus was on search queries alone, it was significant that there were improvements
...
performance.
Related research by Brown et al\(.\)~\cite{brown_language_2020} for example has explored differences in model performance depending on examples provided as
part of the LLM prompt~\cite{brown_language_2020}.
...
ranging from translation and question answering to reasoning tasks.
Notably, few-shot learning models are able to adapt to new tasks with few training data.
It becomes clear that it is important to provide adequate, fitting examples as part of a prompt in order
